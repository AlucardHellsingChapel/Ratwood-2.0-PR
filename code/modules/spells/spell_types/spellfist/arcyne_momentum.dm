...
#define SECONDS_PER_MOMENTUM (6 SECONDS)

/atom/movable/screen/alert/status_effect/buff/arcyne_momentum
	name = "Arcyne Momentum (0)"
	desc = "Melee strikes fuel arcyne power. Melee strikes grant 1 momentum every 2 seconds. At 7 or more the power overcharges - spend it! If it decays unspent, ALL momentum discharges at once, leaving me strained - briefly slowed and vulnerable."
	icon_state = "buff"
...
/datum/status_effect/buff/arcyne_momentum
	...
	var/mutable_appearance/electricity_overlay
...
/datum/status_effect/buff/arcyne_momentum/on_remove()
	UnregisterSignal(owner, list(COMSIG_LIVING_STATUS_STUN, COMSIG_LIVING_STATUS_KNOCKDOWN, COMSIG_HUMAN_MELEE_UNARMED_ATTACK, COMSIG_MOB_ITEM_ATTACK))
	if(is_overcharged)
...
/// Momentum from katars and knuckles - only weapons that run off the unarmed skill.
/datum/status_effect/buff/arcyne_momentum/proc/on_unarmed_item_attack(mob/living/source, mob/living/target, mob/living/user, obj/item/weapon)
	SIGNAL_HANDLER
...
/datum/status_effect/buff/arcyne_momentum/proc/gain_from_melee()
...
/datum/status_effect/buff/arcyne_momentum/proc/on_stunned()
	SIGNAL_HANDLER
	lose_all_momentum("The shock breaks my concentration - all momentum lost!")

/datum/status_effect/buff/arcyne_momentum/proc/on_knockdown()
	SIGNAL_HANDLER
	lose_all_momentum("I hit the ground - all momentum lost!")

/datum/status_effect/buff/arcyne_momentum/proc/lose_all_momentum(message)
	if(stacks <= 0)
		return
	stacks = 0
	refresh_momentum()
...
/datum/status_effect/buff/arcyne_momentum/proc/add_stacks(amount)
	...
	if(old_stacks < overcharge_threshold && stacks >= overcharge_threshold)
		to_chat(owner, span_boldwarning("The power within me strains to be unleashed!"))
...
/datum/status_effect/buff/arcyne_momentum/tick()
	if(stacks <= 0 || world.time - last_stack_time < MOMENTUM_DECAY_DELAY)
		return
	if(world.time - last_decay_time < SECONDS_PER_MOMENTUM)
		return
...
/// Letting an overcharge decay lashes back through the caster instead of fizzling out.
...
/proc/get_arcyne_momentum(mob/living/target)
	if(!istype(target))
		return 0
	var/datum/status_effect/buff/arcyne_momentum/momentum = target.has_status_effect(/datum/status_effect/buff/arcyne_momentum)
	if(!momentum)
		return 0
	return momentum.stacks

#undef MOMENTUM_FILTER
#undef MOMENTUM_DECAY_DELAY
#undef SECONDS_PER_MOMENTUM
